...
module.exports = function i18nTransform(code) {
  ...
  if (
    excludes.length &&
    excludes.some((item) => resourcePath.indexOf(item) === 0)
  ) {
    return code;
  }
  if (
    includes.length &&
    includes.every((item) => resourcePath.indexOf(item) !== 0)
  ) {
    return code;
  }
  ...
  const visitor = {
    ...
    TemplateLiteral(path) {
      ...
      const hasWord = path.node.quasis.some((item) =>
        globalSetting.localePattern.test(item.value.raw),
      );
      if (!hasWord) {
        return;
      }
  ...
  if (transform && dependency && hasLang && !loadedDependency) {
    ...
    const i18nImport = `import ${
      objectPattern ? '{' + name + '}' : name
