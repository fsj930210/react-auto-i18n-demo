...
const localeWordPattern = (word) => {
  ...
  wordByLines.forEach((wordLine) => {
    ...
    let wordMatchPart = '';
    if (lastSpace) {
      wordMatchPart = wordLine.substring(
        firstCharNotSpaceIndex,
        lastSpace.index,
      );
    } else {
...
const createSplitNode = ({ word, wordKeyMap, calle }) => {
  ...
  wordMatchPart = types.callExpression(types.identifier(calle), [
