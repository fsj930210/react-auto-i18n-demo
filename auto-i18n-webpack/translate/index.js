...
const { secretId, secretKey, region, endpoint, source, projectId } =
...
const translateTo = ({ target, textConfig }) => {
...
const createTranslate = (target, source, needFile = true) => {
  return new Promise((resolve, reject) => {
    ...
    const { path: sourcePath, text } = source;
    ...
    const translateLang = (index) => {
      ...
      fs.access(filePath, fs.constants.F_OK, async (err) => {
        ...
        if (err) {
          ...
          deletedKeys = Object.keys(langConfig).filter(
            (key) =>
              !Object.keys(localeConfig).some((localeKey) => localeKey === key),
          );
        ...
        if (Object.keys(translateWordConfig).length) {
          if (!validateLimit(translateWordConfig)) {
            return reject(
              `translate ${item} failed: The terms that need to be translated exceed the free quota for characters!`,
            );
          }
          try {
